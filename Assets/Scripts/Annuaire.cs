...
public class Annuaire : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_isClosing)
        {
            _rect.anchorMin = Vector2.Lerp(_rect.anchorMin, new Vector2(0, 0.5f), _timer / 3);
            _rect.anchorMax = Vector2.Lerp(_rect.anchorMax, new Vector2(0, 0.5f), _timer / 3);
            _rect.anchoredPosition = Vector2.Lerp(_rect.anchoredPosition, new Vector2(50, 0), _timer / 3);
            _rect.localScale = Vector2.Lerp(_rect.localScale, _currentScale / 2, _timer / 3);
            _timer += Time.deltaTime;
        }
        if (_timer > 3)
